...
fn test_task3_storage_integration() {
    ...
    let new_size = storage.state.read().imm_memtables[1].approximate_size();

    assert!(
        new_size == previous_approximate_size,
        "wrong order of memtables? new size {} expected to be the same as prev size {}",
        new_size.to_string(),
        previous_approximate_size.to_string()
    );
